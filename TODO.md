...
**▶ 当前进度**：**任务 50（M4-012，RC）进行中**（2026-08-02，main）：冻结核对 + fake 238 全绿 + 真实 CLI 6/6（CLI 0.31.1）+ 兼容矩阵补 0.31.1 完成，`doc/RC-验收报告.md` 已备——**待用户按报告 §七做 GUI 验收（含 §16 演示脚本 16 步），反馈后勾选 Gate RC 清单并收官第一阶段**
...
- [~] 50. **M4-012** 全量回归与候选构建（RC 前冻结：schema/协议映射/allowlist/版本/文案/范围）｜ 依赖：全部 ｜ 输出：RC
  - 进行中（2026-08-02）：六项冻结核对完毕；fake 238/238 全绿；真实 CLI 6/6 全过（0.31.1）；兼容矩阵已补 0.31.1 行（ADR-001 + AGENTS.md）；release 构建归档与 `doc/RC-验收报告.md` 已备；待用户按报告 §七执行 GUI 验收（§16 演示脚本 16 步）后关闭
